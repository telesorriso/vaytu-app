...
import { approveCreator, rejectCreator, toggleSuspendCreator, type AdminActionState } from '@/app/admin/actions';
...
export function DecisionPanel({
  ...
  const [approveState, approveAction] = useActionState(approveCreator, initialState);
  const [rejectState, rejectAction] = useActionState(rejectCreator, initialState);
  const [suspendState, suspendAction] = useActionState(toggleSuspendCreator, initialState);
  ...
  return (
    <div className="space-y-4 rounded-md border border-zinc-200 p-4 dark:border-zinc-800">
      ...
      <p className="text-xs text-zinc-500">
        Stato attuale candidatura: <strong>{verificationStatus ?? 'nessuna'}</strong>
      </p>
      ...
      {approveState.success && <InfoBanner message="Creator approvato." />}
      ...
      {showReject && (
        <form action={rejectAction} className="space-y-2 border-t border-zinc-200 pt-3 dark:border-zinc-800">
          ...
          {rejectState.success && <InfoBanner message="Candidatura rifiutata." />}
