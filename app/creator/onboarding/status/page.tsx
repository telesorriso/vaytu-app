...
const STATUS_COPY: Record<string, { title: string; body: string }> = {
  pending: {
    ...
    body: 'Il team VAYTU sta verificando il tuo profilo. Ti aggiorneremo appena la revisione è completata.',
  },
  in_review: {
    title: 'Candidatura in revisione',
    body: 'Il team VAYTU sta verificando il tuo profilo. Ti aggiorneremo appena la revisione è completata.',
  ...
  verified: {
    ...
    body: 'Il tuo profilo è verificato. Le Experiences non sono ancora attive su VAYTU — torna presto.',
  },
  rejected: {
    title: 'Candidatura non approvata',
    body: 'La tua candidatura non è stata approvata questa volta.',
  },
};

export default async function CreatorOnboardingStatus() {
  await requireRole('creator');
  const data = await getCreatorOnboardingData();
  if (!data || !hasSubmittedApplication(data)) redirect('/creator/onboarding');
